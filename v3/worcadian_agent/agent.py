...
Usage:
    python -m worcadian_agent.agent [--day N] [--model PROVIDER:MODEL] [--provider PROVIDER]
"""
...
def _main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Use one specific model instead of the MODEL_NAME_1..5 fallback chain, "
            "e.g. 'anthropic:claude-sonnet-5' or 'openai:gpt-4o-mini'"
        ),
    )
    parser.add_argument(
        "--provider",
        default=None,
        help="Explicit provider for --model, if --model doesn't already have a 'provider:' prefix",
    )
